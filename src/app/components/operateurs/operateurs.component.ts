...
export class OperateursComponent {
  ...
  constructor() {
    const input1$ = this.Form.get('input1')!.valueChanges.pipe(
      debounceTime(300),
      takeUntil(this.endStream1$)
    );

    const input2$ = this.Form.get('input2')!.valueChanges.pipe(
      debounceTime(300),
      takeUntil(this.endStream2$)
    );
